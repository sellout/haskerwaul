...
associatedClosure :: forall c a b. (Semicategory c, GaloisConnection c a b) => Proxy b -> a `c` a
associatedClosure Proxy = upperAdjoint @_ @_ @b . lowerAdjoint

associatedKernel :: forall c a b. (Semicategory c, GaloisConnection c a b) => Proxy a -> b `c` b
