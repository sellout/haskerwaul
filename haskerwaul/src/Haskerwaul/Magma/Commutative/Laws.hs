...
data CommutativeMagmaLaws c t a = CommutativeMagmaLaws
  { commutative :: Law c EqualityRelation (t a a) a
  }

commutativeMagmaLaws ::
  (BraidedMonoidalCategory c t, CommutativeMagma c t a) => CommutativeMagmaLaws c t a
commutativeMagmaLaws =
  CommutativeMagmaLaws
    { commutative = commutativity op
    }
