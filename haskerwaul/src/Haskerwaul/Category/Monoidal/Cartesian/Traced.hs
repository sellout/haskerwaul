...
class (CartesianMonoidalCategory c, TracedMonoidalCategory c (Prod c)) =>
      TracedCartesianMonoidalCategory c where
...
cartesianTrace
  :: forall c a b x. (TracedCartesianMonoidalCategory c, Ob c a, Ob c b, Ob c x)
  => Prod c a x `c` Prod c b x -> a `c` b
