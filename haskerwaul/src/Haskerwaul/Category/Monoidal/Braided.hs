...
instance
  (d ~ (->), dt ~ (,), BraidedMonoidalCategory d dt) =>
  BraidedMonoidalCategory (NaturalTransformation c d) (FTensor dt)
  where
  braid =
    Iso
...
instance
  (d ~ (->), BraidedMonoidalCategory d dt) =>
  BraidedMonoidalCategory (DinaturalTransformation d) (BTensor dt)
  where
  braid =
    Iso
