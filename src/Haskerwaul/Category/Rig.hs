...
instance (d ~ (->), Bifunctor c d c p, Bifunctor c d d s, Ob d x) =>
         Bifunctor c d d (These p s x) where
  bimap :: forall a1 b1 a2 b2. (Ob c a1, Ob c b1, Ob d a2, Ob d b2)
        => a1 `c` b1 -> a2 `d` b2 -> These p s x a1 a2 `d` These p s x b1 b2
  bimap f g (These t) =
...
instance (c ~ (->), Bifunctor c c c p, Bifunctor c c c s, Ob c x) =>
         Bifunctor c c c (Alternating p s x) where
  bimap :: forall a1 b1 a2 b2. (Ob c a1, Ob c b1, Ob c a2, Ob c b2)
        => a1 `c` b1 -> a2 `c` b2 -> Alternating p s x a1 a2 `c` Alternating p s x b1 b2
